...
class AP_Logger {
    ...
    public static function log($message, $type = self::TYPE_INFO, $level = self::LEVEL_INFO, $context = []) {
        ...
        // Sanitizar contexto: ocultar claves y tokens sensibles
        $context = self::sanitize_sensitive_data($context);
     ...
     */
    private static function sanitize_sensitive_data(array $data): array {
        $sensitive_keys = [
            'license_key',
            'api_key',
            'key',
        ...
        foreach ($data as $key => $value) {
            ...
            foreach ($sensitive_keys as $sensitive) {
                if (stripos($key, $sensitive) !== false) {
                    $data[$key] = '[HIDDEN]';
                    continue 2; // Saltar al siguiente elemento del array principal
                }
            }

            // Si el valor es un array, sanitizar recursivamente
            if (is_array($value)) {
    ...
    private static function rotate_if_needed($log_file) {
        ...
        // Usar constante definida en el plugin principal
        if (filesize($log_file) > AP_LOG_MAX_SIZE) {
